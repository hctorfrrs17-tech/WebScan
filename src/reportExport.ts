...
export function reportToMarkdown(report: AssessmentReport) {
  ...
  const findings = report.findings.map((item) => `### ${escapeMarkdown(item.title)}\n\n- **Agent:** ${escapeMarkdown(item.specialist)}\n- **Severity:** ${item.severity}\n- **Status:** ${item.status}\n- **Confidence:** ${item.confidence}\n- **Observed:** ${escapeMarkdown(item.evidence)}\n- **Why it matters:** ${escapeMarkdown(item.impact)}\n- **Remediation:** ${escapeMarkdown(item.remediation)}\n- **Verify:** ${escapeMarkdown(item.verification)}${item.reference ? `\n- **Reference:** ${escapeMarkdown(item.reference)}` : ""}`).join("\n\n");
  const agentReadout = report.specialists.map((agent) => `- **${escapeMarkdown(agent.label)}:** ${agent.state}; ${agent.findingCount} finding${agent.findingCount === 1 ? "" : "s"}. ${escapeMarkdown(agent.focus)}`).join("\n");
  const evidence = report.evidenceSummary.ownerEvidenceProvided ? `${report.evidenceSummary.sourceFilesReviewed} redacted owner file(s) reviewed for this report only (${report.evidenceSummary.reviewedFileTypes.join(", ") || "text"}). No file content is included in this export.` : "No owner evidence was supplied; owner-evidence agents remain limited.";
  return `# WebScan defensive posture review\n\n> ${reportPreamble(report)}\n\n| Target | Score | Grade | Attention items | Agents completed | Reviewed at |\n| --- | ---: | --- | ---: | ---: | --- |\n| ${escapeMarkdown(report.target)} | ${report.score}/100 | ${report.grade} | ${attention} | ${report.specialists.filter((agent) => agent.state === "complete").length}/15 | ${report.verifiedAt} |\n\n## Evidence handling\n\n${escapeMarkdown(evidence)}\n\n## Coverage\n\n${report.coverage.map((item) => `- ${escapeMarkdown(item)}`).join("\n")}\n\n## 15-agent readout\n\n${agentReadout}\n\n## Limitations\n\n${report.limitations.map((item) => `- ${escapeMarkdown(item)}`).join("\n")}\n\n## Findings\n\n${findings}\n\n## AI coding remediation brief\n\n\`\`\`text\n${report.generatedPrompt}\n\`\`\`\n`;
}
